...
export default function SocialLoginButtons({ onSuccess, onError }: SocialLoginProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      let retries = 10;
      while (!window.google && retries > 0) {
        await new Promise(resolve => setTimeout(resolve, 500));
        retries--;
      }

      if (!window.google) {
        throw new Error('Google SDK not loaded. Check NEXT_PUBLIC_GOOGLE_CLIENT_ID env var.');
      }
      ...
      if (onError) onError(error instanceof Error ? error.message : 'Google login unavailable');
      setLoading(null);
    }
  };
  ...
  const handleFacebookLogin = async () => {
    ...
    try {
            ...
            if (response.authResponse) {
              window.FB.api('/me', { fields: 'id,name,email' }, async (userInfo: any) => {
                try {
                  ...
                  if (onError) onError(err instanceof Error ? err.message : 'Facebook login failed');
                }
              });
            } else {
      ...
      if (onError) onError('Facebook login unavailable');
      setLoading(null);
    }
  };
  ...
  const handleAppleLogin = async () => {
    ...
    try {
      let retries = 10;
      while (!window.AppleID && retries > 0) {
        await new Promise(resolve => setTimeout(resolve, 500));
        retries--;
      }
      ...
      if (onError) onError('Apple login unavailable');
      setLoading(null);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-center text-swar-text-secondary">
