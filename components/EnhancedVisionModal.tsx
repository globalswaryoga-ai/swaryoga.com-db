    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[95vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-6">
            ...
            <div className="space-y-4 sm:space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm sm:text-base"
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none text-sm sm:text-base"
          ...
          {activeTab === 'guidelines' && (
            <div className="space-y-6">
                ...
                <div className="space-y-3">
                  {formData.guidelines.map((guideline, index) => (
                    <input
                      ...
                      className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                      placeholder={`Guideline ${index + 1}: e.g., Practice daily for consistency`}
                    />
                  ))}
                </div>
                <p className="text-xs text-gray-500 mt-2">💡 Add 4 key principles or guidelines for this vision</p>
              </div>
              ...
              <div>
                  ...
                  {formData.ideas.map((idea, index) => (
                    <div key={index} className="flex gap-2">
                      <input
                        ...
                        className="flex-1 px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent text-sm"
                ...
                <p className="text-xs text-gray-500 mt-2">💭 Brainstorm ideas and notes related to your vision</p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'structure' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold text-gray-900">🏆 Milestones & Goals Structure</h3>
                <button
                  type="button"
              ...
              {formData.milestones.length === 0 ? (
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                  ...
                  <p className="text-gray-600">No milestones added yet. Click "Add Milestone" to get started!</p>
                </div>
              ) : (
                formData.milestones.map((milestone, milestoneIndex) => (
                  <div key={milestone.id} className="border-2 border-purple-200 rounded-lg p-4 sm:p-6 space-y-4 bg-purple-50">
                    ...
                    <div className="bg-white rounded-lg p-4 space-y-4">
                      <h5 className="font-semibold text-gray-900">🎯 Goal Details</h5>
                      
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <label className="text-xs font-medium text-gray-700 mb-1 block">Goal Title</label>
                          <input
                            type="text"
                            value={milestone.goal.title}
                            onChange={(e) => updateGoal(milestone.id, 'title', e.target.value)}
                            placeholder="Enter goal title"
                            className="w-full px-3 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-xs"
                          />
                        </div>
                        <div>
                          <label className="text-xs font-medium text-gray-700 mb-1 block">Amount Needed</label>
                          <input
                            type="text"
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <h5 className="font-semibold text-gray-900">✓ To-Do List</h5>
                        <button
                          type="button"
                      ...
                      {milestone.goal.todos.map((todo) => (
                        <div key={todo.id} className="flex flex-col sm:flex-row sm:items-center gap-2 p-3 bg-white border-2 border-blue-200 rounded-lg">
                          ...
                          <input
                            ...
                            className="flex-1 px-3 py-2 border-2 border-gray-300 rounded focus:ring-2 focus:ring-purple-500 focus:border-transparent text-xs"
                          />
                          <div className="flex items-center gap-1">
                            <select
                              value={todo.priority}
                              onChange={(e) => updateTodo(milestone.id, todo.id, 'priority', e.target.value)}
                              className="px-2 py-1 border-2 border-gray-300 rounded focus:ring-2 focus:ring-purple-500 focus:border-transparent text-xs"
                            >
          ...
          {/* Action Buttons */}
          <div className="flex flex-col sm:flex-row sm:justify-end gap-3 pt-6 border-t border-gray-200 sticky bottom-0 bg-white">
            <button
              type="button"
              onClick={onClose}
