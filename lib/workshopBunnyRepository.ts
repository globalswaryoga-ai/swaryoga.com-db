...
function student(row: any): BunnyWorkshopStudent { if (!row) return null as any; return { ...row, _id: String(row.id), cohortId: row.cohort_id, whatsappJid: row.whatsapp_jid, whatsappNumber: row.whatsapp_number, leadId: row.lead_id, leadNumber: row.lead_number, active: bool(row.active), metadata: parse(row.metadata_json, {}) }; }
function attendance(row: any): BunnyWorkshopAttendance { if (!row) return null as any; return { ...row, _id: String(row.id), cohortId: row.cohort_id, studentId: row.student_id, classDate: row.class_date, joinedAt: row.joined_at, leftAt: row.left_at, joined: bool(row.joined), durationSeconds: Number(row.duration_seconds || 0), attendancePercent: Number(row.attendance_percent || 0), metadata: parse(row.metadata_json, {}) }; }
...
export async function upsertRecording(input: Record<string, any>, recordingId = id()) {
  ...
  if (!rows.rows[0]) {
    rows = await bunnyExecute({ sql: 'SELECT * FROM workshop_recordings_sql WHERE cohort_id = ? AND class_date = ?', args: [input.cohortId, date] });
  }
