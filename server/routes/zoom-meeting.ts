...
import ZoomMeeting, { IZoomMeeting } from '../models/ZoomMeeting';
...
// Add attendee
router.post('/:id/attendee', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
router.get('/upcoming/:workshopId', async (req: Request, res: Response) => {
  try {
    const { workshopId } = req.params;
