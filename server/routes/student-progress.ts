...
router.get('/enrollment/:enrollmentId', async (req: Request, res: Response) => {
  try {
...
router.post('/:id/session-watch', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    let sessionRecord = progress.sessionsCompleted.find((s: any) => s.sessionId === sessionId);
    ...
    progress.totalSessionsCompleted = progress.sessionsCompleted.filter((s: any) => s.isWatched).length;
    ...
    if (enrollment && workshop) {
      progress.completionPercentage = Math.floor(
        (progress.totalSessionsCompleted / workshop.totalSessions) * 100
      );
    }
...
router.get('/:id/can-unlock/:sessionNumber', async (req: Request, res: Response) => {
  try {
    ...
    const session = workshop.sessions.find((s: any) => s.sessionId === sessionNum);
    ...
    if (progress.unlockedSessions.includes(sessionNum)) {
      ...
      if (unlockRules.requiresPreviousCompletion) {
        const prevSession = progress.sessionsCompleted.find(
          (s: any) => s.sessionId === sessionNum - 1 && s.isCompleted
        );
...
// Submit testimony
router.post('/:id/testimony', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
router.post('/:id/complete-session/:sessionNumber', async (req: Request, res: Response) => {
  try {
    const { id, sessionNumber } = req.params;
    ...
    const sessionNum = parseInt(sessionNumber);
    let sessionRecord = progress.sessionsCompleted.find((s: any) => s.sessionId === sessionNum);
    ...
    progress.totalSessionsCompleted = progress.sessionsCompleted.filter((s: any) => s.isCompleted).length;
