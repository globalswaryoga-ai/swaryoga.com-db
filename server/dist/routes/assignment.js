...
router.post('/', async (req, res) => {
    try {
        ...
        const { workshopId, sessionId, title, description, instructions, totalPoints, passingPercentage, submissionDeadlineDays, attachments } = req.body;
...
router.patch('/:id/publish', async (req, res) => {
    try {
        ...
        const assignment = await Assignment.findByIdAndUpdate(id, { isPublished }, { new: true });
        if (!assignment) {
            return res.status(404).json({
                success: false,
                message: 'Assignment not found'
