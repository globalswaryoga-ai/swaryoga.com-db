...
router.put('/:id/status', async (req, res) => {
    try {
        ...
        const enrollment = await Enrollment.findByIdAndUpdate(id, { status, cancelledAt: status === 'cancelled' ? new Date() : undefined }, { new: true });
        if (!enrollment) {
            return res.status(404).json({
                success: false,
                message: 'Enrollment not found'
...
router.post('/:id/certificate', async (req, res) => {
    try {
        ...
        const enrollment = await Enrollment.findByIdAndUpdate(id, {
            status: 'completed',
            certificateUrl,
            certificateNumber,
