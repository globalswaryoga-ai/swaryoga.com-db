#!/usr/bin/env node
// Test sending template via Meta Cloud API with proper app secret
...
async function testMetaSend() {
  ...
  console.log('🔑 App Secret:', appSecret ? 'Found (' + appSecret.substring(0, 8) + '...)' : 'Missing');
  console.log('📞 Sending to:', phone);
  
  // Generate app secret proof
  const appSecretProof = crypto.createHmac('sha256', appSecret).update(accessToken).digest('hex');
  console.log('🔐 App Secret Proof:', appSecretProof.substring(0, 16) + '...');
