        ...
        <main className="flex-1 overflow-auto p-6">
          {isLoading ? (
            ...
            <div className="bg-swar-bg border border-swar-border rounded-lg p-8 text-center">
              <LogIn className="h-12 w-12 text-swar-text-secondary mx-auto mb-4" />
              <p className="text-swar-text-secondary text-lg">No signin data available yet</p>
            </div>
          ) : (
            <div className="bg-white rounded-lg shadow overflow-hidden">
                ...
                <table className="w-full">
                  <thead className="bg-swar-bg border-b border-swar-border">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-sm font-semibold text-swar-text">IP Address</th>
                      <th className="px-6 py-3 text-left text-sm font-semibold text-swar-text">User Agent</th>
                      <th className="px-6 py-3 text-left text-sm font-semibold text-swar-text">Login Time</th>
                    </tr>
                  </thead>
                  <tbody>
                    {signinData.map((log, index) => (
                      <tr key={log._id} className={index % 2 === 0 ? 'bg-white' : 'bg-swar-bg'}>
                        <td className="px-6 py-4 text-sm text-swar-text">{log.email}</td>
                        <td className="px-6 py-4 text-sm text-swar-text-secondary">{log.ipAddress ?? 'N/A'}</td>
                        <td className="px-6 py-4 text-sm text-swar-text-secondary truncate max-w-xs">{log.userAgent ?? 'N/A'}</td>
              ...
              <div className="bg-swar-bg border-t border-swar-border px-6 py-4">
                <p className="text-sm text-swar-text-secondary">
                  Total signin logs: <span className="font-semibold text-swar-text">{signinData.length}</span>
                </p>
              </div>
            </div>
          )}
