...
export default function VisionPage() {
  ...
  useEffect(() => {
    setMounted(true);
    (async () => {
      const saved = await crmPlannerStorage.getVisions();
  ...
  const handleSaveVision = async (visionData: Omit<Vision, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    // Save to database
    try {
      await crmPlannerStorage.saveVisions(updatedVisions);
    } catch (error) {
  ...
  const handleSaveActionPlan = async (actionPlan: any) => {
    const actionPlans = await crmPlannerStorage.getActionPlans();
    actionPlans.push(actionPlan);
    await crmPlannerStorage.saveActionPlans(actionPlans);
