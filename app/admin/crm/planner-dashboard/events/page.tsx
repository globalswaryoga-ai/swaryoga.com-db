...
export default function LifePlannerEventsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        let mongoEvents = await crmPlannerStorage.getEvents();
        ...
        if (mongoEvents.length > 0) {
          ...
          if (localList.length > 0) {
            await crmPlannerStorage.saveEvents(localList);
          }
        }
      } catch {
  ...
  useEffect(() => {
    ...
    (async () => {
      await crmPlannerStorage.saveEvents(events);
    })();
  ...
  const handleSave = async (
    ...
    if (todosToCreate.length > 0) {
      ...
      if (createdTodos.length > 0) {
        ...
        try {
          const existing = await crmPlannerStorage.getTodos();
          await crmPlannerStorage.saveTodos([...(Array.isArray(existing) ? existing : []), ...createdTodos]);
        } catch {
          // ignore
        }
      }
    ...
    if (options.reminder) {
      ...
      try {
        const existing = await crmPlannerStorage.getReminders();
        await crmPlannerStorage.saveReminders([...(Array.isArray(existing) ? existing : []), reminder]);
      } catch {
        // ignore
      }
    }
