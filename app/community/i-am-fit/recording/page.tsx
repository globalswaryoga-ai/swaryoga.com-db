...
export default function IAmFitRecordingsPage() {
  ...
  const checkAuth = async () => {
    try {
      ...
      if (userEmail || user?.mobile) {
        ...
        if (memberData.success && memberData.memberships.includes('i-am-fit')) {
          setHasAccess(true);
        } else {
          setError('You do not have access to I am Fit recordings. Please join the community first.');
        }
      } else {
        router.push('/signin');
      }
  ...
  const fetchRecordings = async () => {
    try {
      ...
      if (data.success) {
        const filteredRecordings = data.recordings?.filter((r: Recording) => r.communityId === 'i-am-fit') || [];
        ...
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between gap-4 mb-3">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-900">I am Fit Recordings</h1>
                <p className="text-slate-500 text-sm">Fitness & wellness recordings</p>
              </div>
            </div>
          </div>
          {/* URL Copy Bar */}
          <div className="flex items-center gap-2 bg-slate-50 rounded-lg p-3 border border-slate-200">
            ...
            <code className="flex-1 text-sm text-slate-700 bg-white px-3 py-1 rounded border border-slate-200 overflow-x-auto font-mono">
              {typeof window !== 'undefined' ? window.location.href : 'swaryoga.com/community/i-am-fit/recording'}
            </code>
            <button
        ...
        {!loading && recordings.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-slate-500">
              I am Fit workshop recordings will appear here soon.
            </p>
          </div>
        )}
