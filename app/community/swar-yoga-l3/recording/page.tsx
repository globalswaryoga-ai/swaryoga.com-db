...
export default function SwarYogaL3RecordingsPage() {
  ...
  const checkAuth = async () => {
    try {
      ...
      if (userEmail || user?.mobile) {
        ...
        if (memberData.success && memberData.memberships.includes('swar-yoga-l3')) {
          setHasAccess(true);
        } else {
          setError('You do not have access to Swar Yoga L-3 recordings. Please join the community first.');
        }
      } else {
        router.push('/signin');
      }
  ...
  const fetchRecordings = async () => {
    try {
      ...
      if (data.success) {
        const filteredRecordings = data.recordings?.filter((r: Recording) => r.communityId === 'swar-yoga-l3') || [];
        ...
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between gap-4 mb-3">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-900">Swar Yoga L-3 Recordings</h1>
                <p className="text-slate-500 text-sm">Swar Yoga Level 3 workshops & sessions</p>
              </div>
            </div>
          </div>
          {/* URL Copy Bar */}
          <div className="flex items-center gap-2 bg-slate-50 rounded-lg p-3 border border-slate-200">
            ...
            <code className="flex-1 text-sm text-slate-700 bg-white px-3 py-1 rounded border border-slate-200 overflow-x-auto font-mono">
              {typeof window !== 'undefined' ? window.location.href : 'swaryoga.com/community/swar-yoga-l3/recording'}
            </code>
            <button
        ...
        {!loading && recordings.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-slate-500">
              Swar Yoga L-3 workshop recordings will appear here soon.
            </p>
          </div>
        )}
