...
export default function AaharShastraRecordingsPage() {
  ...
  const checkAuth = async () => {
    try {
      ...
      // Check membership in Aahar community
      if (userEmail || user?.mobile) {
        ...
        if (memberData.success && memberData.memberships.includes('aahar')) {
          setHasAccess(true);
        } else {
          setError('You do not have access to Aahar Shastra recordings. Please join the community first.');
        }
      } else {
        router.push('/signin');
      }
        ...
        <div className="max-w-6xl mx-auto px-4 py-4">
          ...
          <div className="flex items-center gap-2 bg-slate-50 rounded-lg p-3 border border-slate-200">
            ...
            <code className="flex-1 text-sm text-slate-700 bg-white px-3 py-1 rounded border border-slate-200 overflow-x-auto font-mono">
              {typeof window !== 'undefined' ? window.location.href : 'swaryoga.com/community/aahar-shastra/recording'}
            </code>
            <button
        ...
        {!loading && recordings.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-slate-500">
              Aahar Shastra workshop recordings will appear here soon.
            </p>
          </div>
        )}
