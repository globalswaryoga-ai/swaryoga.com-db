...
// Dual-secret rotation: accept env/default secret PLUS strong secret(s) listed
// (one per line) in .allowed-secrets. Lets us rotate off the public default
// with zero downtime, then drop the default from the file.
const ALLOWED_SECRETS = (() => {
  const set = new Set([BRIDGE_SECRET].filter(Boolean));
  try {
    const extra = require('fs').readFileSync(__dirname + '/.allowed-secrets', 'utf8');
    for (const line of extra.split('\n')) { const v = line.trim(); if (v && !v.startsWith('#')) set.add(v); }
  } catch (e) {}
...
// unsigned }) instead of a plain number once the protobuf decode picks that
// path. Number(longObject) is NaN, which turns into "Invalid Date" once the
// webhook payload is saved to Mongo — the message gets silently rejected and
// never shows up in the inbox. Normalize to plain epoch seconds everywhere
// this gets forwarded or cached.
...
// invalidate the half-finished pairing before we come back, so the next
// attempt gets a real 401 and the bridge wipes the just-created credentials.
const RESTART_REQUIRED_RETRY_DELAY_MS = Number(process.env.WA_BRIDGE_RESTART_REQUIRED_RETRY_DELAY_MS || 500);
const CONNECTION_DROP_RETRY_DELAY_MS = Number(process.env.WA_BRIDGE_CONNECTION_DROP_RETRY_DELAY_MS || 30000);
const RATE_LIMIT_RETRY_BASE_MS = Number(process.env.WA_BRIDGE_RATE_LIMIT_RETRY_BASE_MS || 300000);
...
async function startSocket(sessionKey, ownerUserId = sessionKey, tenantId = null) {
  ...
  try {
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        if (shouldReconnect) {
          ...
          const isQrTimeout = !isRestartRequired && (statusCode === 408 || hadRecentQR);
          ...
          let delay;
          if (isRestartRequired) delay = RESTART_REQUIRED_RETRY_DELAY_MS;
          else if (isQrTimeout) delay = QR_RETRY_DELAY_MS;
          else if (isConnectionConflict) delay = Math.min(10000 + session.retryCount * 5000, 120000);
          else if (isRateLimited) delay = Math.min(RATE_LIMIT_RETRY_BASE_MS + session.retryCount * 60000, 900000);
          else if (isConnectionDrop) delay = CONNECTION_DROP_RETRY_DELAY_MS;
      ...
      for (const msg of msgs) {
        try {
          ...
          if (msg.key.remoteJid === 'status@broadcast') {
            try {
              ...
              const statusEntry = {
                ...
                timestamp: normalizeMsgTimestamp(msg.messageTimestamp),
          ...
          const webhookPayload = {
            ...
            text, body: text, messageId, timestamp: normalizeMsgTimestamp(msg.messageTimestamp),
          ...
          const msgEntry = {
            ...
            text: displayText, type: mediaInfo?.kind || 'text', timestamp: normalizeMsgTimestamp(msg.messageTimestamp),
    ...
    if (!session.intentionalDisconnect) {
      session.retryCount++;
      const delay = Math.min(session.retryCount * 15000, 300000);
      session.clearReconnectTimer();
      session.reconnectTimer = setTimeout(() => { session.isStarting = false; startSocket(session.sessionKey, session.ownerUserId, session.tenantId); }, delay);
    }
  } finally {
...
app.post('/send', async (req, res) => {
  ...
  try {
    // Human-like typing simulation before sending (anti-robotic)
    try {
      await session.sock.presenceSubscribe(jid);
      await session.sock.sendPresenceUpdate('composing', jid);
      var _sample = (type === 'media') ? String(caption || message || '') : String(message || '');
      var _typeMs = Math.min(Math.max(_sample.length * 55, 900), 4000);
      await new Promise(function (r) { setTimeout(r, Math.floor(_typeMs * (0.7 + Math.random() * 0.6))); });
      await session.sock.sendPresenceUpdate('paused', jid);
    } catch (_) { /* presence best-effort, never block send */ }
