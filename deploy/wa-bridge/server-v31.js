...
async function bootstrap() {
  console.log('=== SWAR YOGA BRIDGE v3.1 ===');
...
app.get('/qr', (req, res) => {
  if (qrCodeData) {
    qrcode.toDataURL(qrCodeData, (err, url) => {
      res.send('<html><body style="background:#111;color:#fff;text-align:center;padding:50px;font-family:sans-serif;"><div style="background:#fff;color:#000;display:inline-block;padding:30px;border-radius:20px;box-shadow:0 10px 40px rgba(0,0,0,0.5);"><h2>Swar Yoga Bridge v3.1</h2><img src="' + url + '" style="width:300px;"><p>Scan to connect</p></div></body></html>');
    });
...
app.post('/send', async (req, res) => {
  ...
  try {
    ...
    // Use sendMessage directly - handles new chats automatically
