...
Main WhatsApp Bridge Server - Runs on port 3333
...
app.get('/qr', authMiddleware, (req, res) => {
  if (!qrCode) {
    return res.status(400).json({
      ...
      status: 'disconnected',
      hasQr: false,
      message: 'QR is not available yet. Wait a few seconds or restart.'
    });
  }
  res.json({
    ok: true,
...
```json
{
  "name": "wa-bridge",
  "version": "1.0.0",
  "description": "WhatsApp Web Bridge for VPS deployment",
  "main": "server.js",
  "scripts": {
...
```bash
# Allow browser requests from your CRM domains
